...
function LoginContent() {
    ...
    const handleResetPassword = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setResetMsg(data.message || "Reset link sent! Please check your email.");
            setIsResetLoading(false);
        } catch (err) {
            setError("An unexpected error occurred. Please try again later.");
            setIsResetLoading(false);
                ...
                >
                    ...
                    {showForgotPassword ? (
                        <form onSubmit={handleResetPassword} className="space-y-4 text-left">
                            ...
                            {resetMsg && (
                                <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="p-3 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-700 text-sm text-center">
                                    {resetMsg}
                                </motion.div>
                            )}
                            ...
                            <button 
                                type="button"
                                onClick={() => { setShowForgotPassword(false); setResetMsg(null); setError(null); }}
