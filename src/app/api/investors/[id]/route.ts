    ...
    try {
        ...
        for (const bucket of Object.keys(pathsByBucket)) {
            ...
            if (paths.length > 0) {
                ...
                if (storageError) {
                    console.error(`Failed to delete storage files for investor ${id} in bucket ${bucket}:`, storageError);
                }
            }
        }
        ...
        return NextResponse.json({ success: true, message: `Successfully deleted investor and associated files.` });
    } catch (error: any) {
